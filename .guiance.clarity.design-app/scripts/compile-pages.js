...
async function main() {
  ...
  for (const pageFilePath of glob.sync('../*.md')) {
    ...
    pages.push({ filename, number, title });

    writeText(`${filename}.component.html`, await compileMarkdown(body));
    writeText(`${filename}.component.ts`, getPageComponentTs({ filename, title }));
  }
  ...
  writeJson('numbered-pages.json', numberedPages);
...
function getPageComponentTs({ filename, title }) {
  return `
...
export class PageComponent implements OnInit {
  ...
  ngOnInit() {
    this.title.setTitle('${title ? `${title} | Clarity Guidance` : 'Clarity Guidance'}');
  }
}
`;
}
